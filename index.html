...
<head>
    ...
    <!-- PWA Meta Tags -->
    <meta name="theme-color" content="#F7631B">
    <meta name="apple-mobile-web-app-capable" content="yes">
    <meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">
    <meta name="apple-mobile-web-app-title" content="GullyScorer">
    ...
    <style type="text/tailwindcss">
        ...
        .bg-brand-primary { 
            background: linear-gradient(135deg, #F7631B, #fb923c, #ec4899, #F7631B);
            background-size: 300% 300%;
            animation: brandShimmer 6s ease infinite;
        }
        ...
        /* Animated App Background - Applied to the main container */
        .animated-app-bg {
             background: linear-gradient(-45deg, #ffffff, #fff7ed, #ffedd5, #fff0f5, #ffffff);
             background-size: 400% 400%;
             animation: appGradient 15s ease infinite;
        }
        ...
        /* Coin Button Spin Animation */
        @keyframes coin-y-spin {
            0% { transform: rotateY(0deg); }
            100% { transform: rotateY(360deg); }
        }
        .coin-spin-anim {
            display: inline-block;
            animation: coin-y-spin 2s infinite linear;
        }

        /* Prevent elastic scrolling and fix white gap at bottom */
        html {
...
<body class="bg-gray-100 min-h-screen font-sans text-gray-900 overscroll-y-none">

    <!-- App Container -->
    <div id="app" class="max-w-xl mx-auto animated-app-bg min-h-screen shadow-2xl flex flex-col relative border-x border-gray-200">
        ...
        <header class="bg-brand-primary text-white p-4 shadow-md sticky top-0 z-50">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        <h1 class="text-lg font-bold tracking-wider leading-tight uppercase">Live Cricket Scorer</h1>
                        <div class="flex items-center gap-2">
                ...
                <!-- Install Button -->
                <button id="install-btn" class="hidden bg-white/20 hover:bg-white/30 p-2 rounded-full text-white transition-colors shadow-inner" title="Install App">
        ...
        <div id="setup-screen" class="p-6 flex-grow flex flex-col">
            ...
            <div class="space-y-4 flex-grow">
                ...
                <div class="flex justify-end -mt-2">
                    <button onclick="openTossModal()" class="group bg-white border border-orange-200 text-gray-700 hover:text-[#F7631B] hover:border-orange-300 px-4 py-1.5 rounded-full shadow-sm hover:shadow-md transition-all flex items-center gap-2 perspective-[500px]">
                        <span class="relative flex h-2 w-2">
                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-orange-400 opacity-75"></span>
                            <span class="relative inline-flex rounded-full h-2 w-2 bg-orange-500"></span>
                        </span>
                        <span class="text-[10px] font-black uppercase tracking-widest">Digital Toss</span>
                        <span class="text-sm coin-spin-anim group-hover:[animation-play-state:paused]">🪙</span>
                    </button>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="flex items-center border border-gray-300 rounded-xl bg-white overflow-hidden shadow-sm">
                            <button onclick="document.getElementById('max-overs').stepDown()" class="px-4 py-3 bg-gray-50 hover:bg-gray-200 text-gray-600 font-black border-r transition-colors active:bg-gray-300">-</button>
                            <input type="number" id="max-overs" value="5" min="1" max="999" class="w-full p-3 text-center bg-transparent outline-none font-bold text-gray-800" readonly>
                            <button onclick="document.getElementById('max-overs').stepUp()" class="px-4 py-3 bg-gray-50 hover:bg-gray-200 text-gray-600 font-black border-l transition-colors active:bg-gray-300">+</button>
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-600 mb-1 font-bold">Players</label>
                        <div class="flex items-center border border-gray-300 rounded-xl bg-white overflow-hidden shadow-sm">
                            <button onclick="document.getElementById('player-count').stepDown()" class="px-4 py-3 bg-gray-50 hover:bg-gray-200 text-gray-600 font-black border-r transition-colors active:bg-gray-300">-</button>
                            <input type="number" id="player-count" value="11" min="2" max="15" class="w-full p-3 text-center bg-transparent outline-none font-bold text-gray-800" readonly>
                            <button onclick="document.getElementById('player-count').stepUp()" class="px-4 py-3 bg-gray-50 hover:bg-gray-200 text-gray-600 font-black border-l transition-colors active:bg-gray-300">+</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-2">
                    <button onclick="showHistory()" class="group w-full bg-white hover:bg-gray-50 text-gray-700 font-bold py-4 rounded-xl border border-gray-200 transition-all active:scale-95 uppercase text-xs flex items-center justify-center gap-3 shadow-md hover:shadow-lg relative overflow-hidden">
                        <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-red-500 via-red-400 to-red-500"></div>
                        <div class="flex items-center gap-2 px-3 py-1 bg-red-50 text-red-600 rounded-full border border-red-100">
                            ...
                            <span class="text-[10px] font-black tracking-wider">LIVE</span>
                        </div>
                        <span class="text-gray-600 group-hover:text-gray-800">Scores & History</span>
                    </button>
                </div>
            </div>
        </div>
        ...
        <!-- Wicket Type Modal -->
        <div id="wicket-type-modal" class="hidden fixed inset-0 z-[95] flex items-center justify-center p-4">
            <div class="absolute inset-0 bg-black/80 backdrop-blur-sm" onclick="document.getElementById('wicket-type-modal').classList.add('hidden')"></div>
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 modal-enter">
                <div class="text-center mb-4">
                    <h3 class="text-lg font-black text-gray-800 uppercase">How's That?</h3>
                    <p class="text-xs text-gray-500">Select dismissal type</p>
                </div>
                
                <div id="wicket-main-options" class="grid grid-cols-2 gap-3 mb-4">
                    <button onclick="processWicketType('caught')" class="p-3 bg-blue-50 text-blue-700 font-bold rounded-xl hover:bg-blue-100 border border-blue-200">Catch Out</button>
                    <button onclick="processWicketType('bowled')" class="p-3 bg-red-50 text-red-700 font-bold rounded-xl hover:bg-red-100 border border-red-200">Bowled</button>
                    <button onclick="processWicketType('lbw')" class="p-3 bg-gray-50 text-gray-700 font-bold rounded-xl hover:bg-gray-100 border border-gray-200">LBW</button>
                    <button onclick="processWicketType('stumped')" class="p-3 bg-gray-50 text-gray-700 font-bold rounded-xl hover:bg-gray-100 border border-gray-200">Stumped</button>
                    <button onclick="showRunOutOptions()" class="col-span-2 p-3 bg-orange-50 text-[#F7631B] font-bold rounded-xl hover:bg-orange-100 border border-orange-200">Run Out</button>
                </div>

                <!-- Run Out Specific Options -->
                <div id="runout-options" class="hidden space-y-4 bg-gray-50 p-4 rounded-xl border border-gray-200">
                    <div>
                        <label class="block text-[10px] font-bold text-gray-500 uppercase mb-2">Runs Completed</label>
                        <div class="grid grid-cols-4 gap-2">
                            <button onclick="selectRunOutRuns(0)" id="btn-ro-run-0" class="p-3 bg-white text-gray-700 border border-gray-200 rounded-lg font-bold hover:bg-gray-100">0</button>
                            <button onclick="selectRunOutRuns(1)" id="btn-ro-run-1" class="p-3 bg-white text-gray-700 border border-gray-200 rounded-lg font-bold hover:bg-gray-100">1</button>
                            <button onclick="selectRunOutRuns(2)" id="btn-ro-run-2" class="p-3 bg-white text-gray-700 border border-gray-200 rounded-lg font-bold hover:bg-gray-100">2</button>
                            <button onclick="selectRunOutRuns(3)" id="btn-ro-run-3" class="p-3 bg-white text-gray-700 border border-gray-200 rounded-lg font-bold hover:bg-gray-100">3</button>
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-500 uppercase mb-2">Who is Out?</label>
                        <div class="grid grid-cols-2 gap-2">
                            <button id="btn-ro-striker" onclick="selectRunOutTarget('striker')" class="p-2 text-xs font-bold border rounded-lg bg-white truncate">Striker</button>
                            <button id="btn-ro-nonstriker" onclick="selectRunOutTarget('nonStriker')" class="p-2 text-xs font-bold border rounded-lg bg-white truncate">Non-Striker</button>
                        </div>
                    </div>
                    <button onclick="processRunOut()" class="w-full bg-brand-primary text-white font-bold py-2 rounded-lg uppercase text-xs shadow-md">Confirm Run Out</button>
                </div>
            </div>
        </div>
        ...
        <div id="new-batsman-modal" class="hidden fixed inset-0 z-[95] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 modal-enter">
                ...
                <button onclick="submitNewBatsman()" class="w-full bg-brand-primary text-white font-bold py-3 rounded-xl shadow-lg uppercase tracking-widest">Update Batsman</button>
            </div>
        </div>
        ...
        <div id="nb-runs-modal" class="hidden fixed inset-0 z-[95] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 modal-enter text-center">
                <button onclick="document.getElementById('nb-runs-modal').classList.add('hidden')" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 z-10">
        ...
        <div id="toss-modal" class="hidden fixed inset-0 z-[90] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center overflow-hidden modal-enter">
                <button onclick="closeTossModal()" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 z-10">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
                ...
                <div id="toss-decision-select" class="hidden space-y-3 mt-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <button onclick="finalizeToss('bat')" class="p-3 bg-blue-50 text-blue-600 border border-blue-200 rounded-xl font-bold text-sm hover:bg-blue-100 transition-colors">🏏 Bat</button>
                        <button onclick="finalizeToss('bowl')" class="p-3 bg-green-50 text-green-600 border border-green-200 rounded-xl font-bold text-sm hover:bg-green-100 transition-colors">⚾ Bowl</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="history-screen" class="hidden p-6 flex-grow overflow-y-auto">
            ...
            <div class="relative mb-4">
                ...
                <input type="search" inputmode="numeric" id="history-search" oninput="renderHistoryList()" class="block w-full pl-10 p-3 border border-gray-300 rounded-xl bg-gray-50 text-gray-900 focus:ring-2 focus:ring-[#F7631B] outline-none font-bold placeholder-gray-400 transition-colors" placeholder="Search Match ID (e.g. 84392)">
            </div>
        ...
        <div id="viewer-screen" class="hidden flex-grow flex flex-col relative overflow-y-auto">
             <!-- Reduced horizontal padding from p-4 to px-2 py-4 to give more width to control bar -->
             <div class="bg-gray-900 text-white px-2 py-4 text-center sticky top-0 z-10 shadow-lg">
                <!-- Header with Teams, Target and Share -->
                <div class="flex justify-between items-start mb-4 px-2">
                    <div class="flex flex-col text-left">
                        <span id="v-display-teams" class="text-xs font-bold text-gray-400 uppercase tracking-widest">Loading...</span>
                        <span id="v-display-match-id" class="text-[10px] font-mono text-gray-500 mt-0.5">ID: --</span>
                    </div>
                    <div class="flex items-center gap-2">
                ...
                <div class="flex justify-between items-end mb-2">
                    <div>
                        <div id="v-start-time-label" class="text-[10px] text-gray-500 uppercase tracking-widest font-medium">Started: --:--</div>
                        <div id="v-partnership" class="text-[10px] text-[#F7631B] mt-1 font-bold uppercase tracking-wider">Partnership: 0 (0)</div>
                    </div>
                    <div class="text-right">
                        <div class="text-5xl font-black leading-none"><span id="v-runs">0</span>/<span id="v-wickets">0</span></div>
                        <div class="text-lg text-gray-400 font-bold leading-none mt-1">Overs: <span id="v-overs">0.0</span> / <span id="v-limit">5</span></div>
                    </div>
                </div>

                <!-- Viewer Player Bar -->
                <!-- Removed w-1/2, used flex-1 for better spacing -->
                <div class="mt-4 bg-gray-800 rounded-lg p-3 text-xs flex justify-between items-center border border-gray-700 shadow-inner">
                    <div class="flex flex-col text-left gap-2 flex-1 min-w-0 pr-2">
                        <div class="font-bold text-white flex items-center gap-1 overflow-hidden whitespace-nowrap">
                            🏏 <span id="v-label-striker" class="truncate">Striker</span> <span class="text-[#F7631B] text-[10px] ml-1">*</span>
                        </div>
                        <div class="text-gray-400 flex items-center gap-1 overflow-hidden whitespace-nowrap">
                            🏃 <span id="v-label-nonStriker" class="truncate">Non-Striker</span>
                        </div>
                    </div>
                    <div class="flex-1 text-right min-w-0 pl-2">
                        ...
                        <div class="font-bold text-white flex items-center justify-end gap-1 overflow-hidden whitespace-nowrap">
                            <span id="v-label-bowler" class="truncate">Bowler</span> ⚾
                        </div>
                    </div>
                </div>
            ...
            <div id="v-break-overlay" class="hidden absolute inset-0 z-10 break-overlay flex flex-col items-center justify-center text-white text-center p-6">
                <div id="v-overlay-icon" class="text-4xl mb-4">☕</div>
            ...
            <!-- Viewer Commentary Section -->
            <div class="p-4 bg-white border-b border-gray-200">
                <h3 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Commentary</h3>
                <div id="viewer-commentary" class="max-h-60 overflow-y-auto space-y-2 text-sm text-gray-600 bg-gray-50 p-2 rounded-lg border border-gray-100">
                    <p class="text-xs text-gray-400 italic">No commentary yet.</p>
                </div>
            </div>
        ...
        <div id="scoring-screen" class="hidden flex-grow flex flex-col relative">
            <!-- Reduced horizontal padding from p-4 to px-2 py-4 to give more width to control bar -->
            <div class="bg-gray-900 text-white px-2 py-4 text-center">
                <!-- Header with Teams, Target and Share -->
                <div class="flex justify-between items-start mb-4 px-2">
                    <div class="flex flex-col text-left">
                        <span id="display-teams" class="text-xs font-bold text-gray-400 uppercase tracking-widest">Loading...</span>
                        <span id="display-match-id" class="text-[10px] font-mono text-gray-500 mt-0.5">ID: --</span>
                    </div>
                    <div class="flex items-center gap-2">
                ...
                <div class="flex justify-between items-end mb-2">
                    <div>
                        <div id="start-time-label" class="text-[10px] text-gray-500 uppercase tracking-widest font-medium">Started: --:--</div>
                        <div id="partnership-display" class="text-[10px] text-[#F7631B] mt-1 font-bold uppercase tracking-wider">Partnership: 0 (0)</div>
                    </div>
                    <div class="text-right">
                        <div class="text-5xl font-black leading-none"><span id="runs">0</span>/<span id="wickets">0</span></div>
                        <div class="text-lg text-gray-400 font-bold leading-none mt-1">Overs: <span id="overs">0.0</span> / <span id="limit">5</span></div>
                    </div>
                </div>
                
                <!-- Player Control Bar -->
                <!-- Removed w-1/2 to allow flex-1 to take available space naturally -->
                <div class="mt-4 bg-gray-800 rounded-lg p-2 text-xs flex justify-between items-center border border-gray-700">
                    <div class="flex flex-col text-left gap-2 flex-1 min-w-0">
                        <div onclick="openPlayerModal('striker')" class="cursor-pointer font-bold hover:text-[#F7631B] transition-colors flex items-center gap-1 group overflow-hidden whitespace-nowrap">
                            🏏 <span id="label-striker" class="border-b border-dotted border-gray-600 truncate">Striker</span> <span class="text-[#F7631B] ml-1">*</span>
                        </div>
                        <div onclick="openPlayerModal('nonStriker')" class="cursor-pointer text-gray-400 hover:text-white transition-colors flex items-center gap-1 group overflow-hidden whitespace-nowrap">
                            🏃 <span id="label-nonStriker" class="border-b border-dotted border-gray-600 truncate">Non-Striker</span>
                        </div>
                    </div>
                    ...
                    <div class="flex-1 text-right min-w-0">
                        ...
                        <div onclick="openPlayerModal('bowler')" class="cursor-pointer font-bold hover:text-[#F7631B] transition-colors flex items-center justify-end gap-1 overflow-hidden whitespace-nowrap">
                            <span id="label-bowler" class="border-b border-dotted border-gray-600 truncate">Bowler</span> ⚾
                        </div>
                    </div>
                </div>
            ...
            <!-- Scoring Commentary Section -->
            <div class="px-4 pb-4 bg-white border-t border-gray-100">
                <h3 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-2 mt-4">Recent Balls</h3>
                <div id="scoring-commentary" class="h-32 overflow-y-auto space-y-2 text-sm text-gray-600 bg-gray-50 p-2 rounded-lg border border-gray-100">
                    <p class="text-xs text-gray-400 italic">No commentary yet.</p>
                </div>
            </div>
        ...
        <div id="confirm-modal" class="hidden fixed inset-0 z-[80] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all scale-100 animate-in fade-in zoom-in duration-200">
                <button onclick="document.getElementById('confirm-cancel-btn').click()" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 z-10">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
    ...
    <!-- Firebase SDKs -->
    <script type="module" src="cricket.js"></script>
</body>
</html>
